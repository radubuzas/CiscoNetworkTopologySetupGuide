...
\noindent \textbf{RouterName (DHCP Server):}
...
(dhcp-config)# dns-server IpDnsServer
(dhcp-config)# domain-name DomainName
\end{minted}

\noindent \textbf{RouterName (DHCP Relay):}
\begin{minted}[bgcolor=lightgray,fontsize=\small,breaklines]{bash}
...
(config)# ntp authentication-key 1 MD5 NtpPassword
\end{minted}
